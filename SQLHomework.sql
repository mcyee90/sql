...
select last_name, count(last_name) as counts from actor group by last_name having count(last_name) > 1;
...
update actor set first_name = 'GROUCHO' where actor_id in (select actor_id from (select actor_id from actor where first_name = "HARPO" and last_name = "Williams") as aid);
